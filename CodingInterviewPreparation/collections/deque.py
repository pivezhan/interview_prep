...
def deque_functions(d, cmd, val=None):
    if cmd == "append":
        d.append(val)
    elif cmd == "appendleft":
        d.appendleft(val)
...
if __name__ == '__main__':
    ...
    for _ in range(N):
        ...
        if len(command) == 2:
            cmd, val = command
            val = int(val)
            deque_functions(d, cmd, val)
        else:
            cmd = command[0]  # Extract the string command
            deque_functions(d, cmd)
    
    # Final step: Print all elements in deque separated by space
    print(*d)